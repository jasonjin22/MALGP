...
@LastEditTime: 2020-06-24 10:01:13
...
def read_data(data_set, conf):
    ...
    for i in range(len(temp_train_set)):
        if temp_train_set[i].get(-1):
            Y_train.append([-1])
            temp = temp_train_set[i][-1]
            X_1.append(temp[0])
            X_2.append(temp[1])
            # c.append(temp_train_set[i][-1])
    ...
    for i in range(len(temp_val_set)):
        if temp_val_set[i].get(-1):
            ...
            X_2.append(temp[1])
            # X_val.append(temp_val_set[i][1])
    X_val.append(np.array(X_1))
    X_val.append(np.array(X_2))
...
@Author: your name
@Date: 2020-06-16 21:16:30
@LastEditTime: 2020-06-24 09:00:52
@LastEditors: Please set LastEditors
@Description: In User Settings Edit
@FilePath: /ALMVL/read_data.py
'''
